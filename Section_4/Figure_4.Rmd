...
If $k_u > \frac{\ell}{2\pi}$, the set of parameters will make $(\overline{c}, \overline{\rho})$ unstable in the domain length $\ell$. Given a fixed set of parameters $d_1, d_2, \overline{c}, \delta$ and deposition function $f(c)$, we can vary $k$ to find the most unstable wave number.
...
for i in range(len(d1Array)):
    ...
    for f in range(0,len(wave)):
        p = d2*c/(DM.atanDep2(c,0.01,0.05,.03)) #Calculate rho bar
        fp = (DM.atanDep2(c+0.0001,0.01,0.05,.03) - DM.atanDep2(c-0.0001,0.01,0.05,.03))/(2*(0.0001)) #Approximate f'(c)
        d3 = p*fp - d2
        k = wave[f]
...
plt.xlabel(r'Wave Number, $k$',fontsize=20)
...
wave = np.linspace(0,L/5,100) #Wave numbers to test (need end point to be greater than L/(2*pi))
...
for i in range(len(d1Array)):
    MM = 0
    for j in range(len(d2Array)):
        if (stableMat[i,j] > L/(2*pi) and MM==0):#Given d_1, find first d2 such that the system is unstable
...
for i in range(len(d1Array)):
    MM = 0
    for j in range(len(d2Array)):
        if (stableMat2[i,j] > L/(2*pi) and MM==0): #Given d_1, find first d2 such that the system is unstable
            StableLine.append(d2Array[j])
            MM=1
...
ax.tick_params(labelsize=18)
ax.set_title('Stability Regions for $\ell = {0}$, $\delta = {1}$'.format(L,delta), fontsize = 20)
ax.text(.1,.8,'Unstable',color='White',fontsize=25)
ax.text(.6,.4,'Stable',color='Black',fontsize=25)

plt.savefig('./Images/Figure_4d')
```
