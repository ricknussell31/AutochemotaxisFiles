...
As well, this performs Fourier analysis on the plankton density, by showing the magnitude of the wave numbers in the Fourier domain. Define $E(k,t_0) = \left|\mathcal{F}(\rho(x,t_0))(k)\right| + 1$, where $\mathcal{F}$ is the Fourier transform. We then plot $\log(E(K))$ to show the most unstable wave number (the addition of 1 is to prevent any issues with the logarithm).


The variables for the 1D system include:

depFcn - The deposition function from Methods.Deposition that will be performing the autochemotaxis. You have currently three option of constantDep, atanDep2, and linAtanDep2 (see Figure 3 or /Methods/Deposition for more info)
...
```{python}
Swimmers = PM.PlankChem(depFcn=DM.atanDep2,left=0,right=5,d1=.2,d2=5,N=221,
                        dt=0.004,depMaxStr=0.01, depThreshold=0.05, depTransWidth=0.03, delta = 0.01)


NonDTime = 1000 #The total non-dimensional time the simulation will run for
...
K = 6 #Closest integer to most unstable wave number
...
omega = fft(PA[0])
axs[2,0].plot(fftshift(freq),np.log(abs(fftshift(omega))+1),color='g',linewidth=2.5)

axs[2,0].axvline(x=K,linestyle='dashed',color='k',linewidth=2.5) #Plot dotted lines at most unstable wave number
axs[2,0].axvline(x=-K,linestyle='dashed',color='k',linewidth=2.5) #Plot dotted lines at most unstable wave number
axs[0,0].ticklabel_format(style='sci', scilimits=(4,4),axis='y',useMathText=True)
axs[1,0].ticklabel_format(style='sci', scilimits=(-2,-2),axis='y')
...
axs[0,0].set_ylim(1e3,14e3)
axs[2,0].set_ylim(-0.5,20)
axs[1,0].set_ylim(1.0e-1,1.4e-1)
...
axs[2,0].set_ylabel(r'$\log(E(k))$',fontsize=22,color='g')
axs[1,0].locator_params(nbins=3)
axs[0,0].locator_params(nbins=3)
axs[2,0].locator_params(nbins=3)
...
axs[0,1].ticklabel_format(style='sci', scilimits=(3,3),axis='y')
axs[1,1].ticklabel_format(style='sci', scilimits=(-2,-2),axis='y')
...
omega = fft(PA[k])
axs[2,2].plot(fftshift(freq),np.log(abs(fftshift(omega))+1),color='g', linewidth=2.5)
...
axs[0,2].set_title(r'$t = {0}$'.format(round(Swimmers.dt*k)),fontsize=25)
...
axs[2,2].locator_params(nbins=3)
...
omega = fft(PA[k])
axs[2,3].plot(fftshift(freq),np.log(abs(fftshift(omega))+1),color='g',linewidth=2.5)

axs[0,3].ticklabel_format(style='sci', scilimits=(3,3),axis='y')
axs[1,3].ticklabel_format(style='sci', scilimits=(-2,-2),axis='y')
...
axs[0,3].set_title(r'$t = {0}$'.format(round(Swimmers.dt*k)),fontsize=25)
axs[1,3].locator_params(nbins=3)
axs[0,3].locator_params(nbins=3)
...
axs[2,3].locator_params(nbins=3)
