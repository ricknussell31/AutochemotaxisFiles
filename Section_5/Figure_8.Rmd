...
ax.tick_params(labelsize=18)
ax.set_title('Stability Regions for $\ell = {0}$, $\delta = {1}$'.format(L,delta), fontsize = 20)
ax.text(.2,.7,'Unstable',color='White',fontsize=25)
ax.text(.7,.2,'Stable',color='Black',fontsize=25)
...
```{python}
plt.figure(figsize=(7.6,5.4),tight_layout = True)
modd = [10,50,100,200] #Values of d2 to be simulated (decay constant)
colors = ['#648FFF','#785EF0','#DC267F','#FE6100','#FFB000'] #Using IBM Colorblind Scheme 
markers = ['o','p','s','^','D']
i = 0
K = 4
...
plt.xlabel(r'Magnitude of Wave Number, $|\vec{k}|$',fontsize=20)
plt.ylabel(r'$R_N(|\vec{k}|)$',fontsize=28,labelpad=-10)
plt.title(r'Maximal Real Part of Eigenvalues: $d_1 = d_2 = 1$', fontsize=20)
plt.legend(loc=0)
plt.xlim(0,5)
plt.ylim(-.75,1)
plt.tick_params(labelsize=18)
